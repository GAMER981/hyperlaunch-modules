    ...
    useCoreConfigs := IniReadCheck(settingsFile, "Settings", "UseCoreConfigs","true",,1)
    ...
    globalRetroCfg := configFolder . "\retroarch.cfg"
    ...
    If FileExist(systemRetroCfg) {  ; check for system cfg first
            ...
            foundCfg := 1
    } Else If (useCoreConfigs && FileExist(coreRetroCfg)) { ; 2nd option is a core config
    ...
    If foundCfg {
            ...
            raCfgHasChanges :=
            If overlay {
                    WriteProperty(retroCFG, "input_overlay", overlay,1,1)
                    raCfgHasChanges := 1
            }
            If videoShader {
                    WriteProperty(retroCFG, "video_shader", videoShader,1,1)
                    raCfgHasChanges := 1
            }
            If aspectRatioIndex {
                    WriteProperty(retroCFG, "aspect_ratio_index", aspectRatioIndex,1,1)
                    raCfgHasChanges := 1
            }
            If customViewportWidth {
                    WriteProperty(retroCFG, "custom_viewport_width", customViewportWidth,1,1)
                    raCfgHasChanges := 1
            }
            If customViewportHeight {
                    WriteProperty(retroCFG, "custom_viewport_height", customViewportHeight,1,1)
                    raCfgHasChanges := 1
            }
            If customViewportX {
                    WriteProperty(retroCFG, "custom_viewport_x", customViewportX,1,1)
                    raCfgHasChanges := 1
            }
            If customViewportY {
    ...
    If (RegExMatch(ident, "LibRetro_N64|LibRetro_NES|LibRetro_LYNX|LibRetro_PSX") || RegExMatch(ident, "LibRetro_NES") && (InStr(core, "nestopia_libretro"))) {     ; these systems will use an ini to store game specific settings
            ...
            If InStr(ident, "LibRetro_N64") {       ; Nintendo 64
                    ...
                    mednafenOptionsCFGFile := CheckFile(configFolder . "\" . LibRetro_PSX . ".dll.cfg", "Could not find core configuration file in config directory: """ . configFolder . "\" . LibRetro_PSX . ".dll.cfg""")
